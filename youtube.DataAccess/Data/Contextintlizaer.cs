    ...
    public static class Contextintlizaer
    {
        public static async Task Initialize(Context context, UserManager<AppUser> userManager, RoleManager<AppRole> roleManager)
        {
            ...
            try 
            {
                ...
                {
                    var existingChannel = await context.Channals.FirstOrDefaultAsync(c => c.AppUserId == u.Id);
                    if (existingChannel == null)
                    {
                        ...
                        context.Channals.Add(newChannel);
                    }
                }
                await context.SaveChangesAsync();

                var channels = await context.Channals.ToListAsync();
                var categories = await context.Categories.ToListAsync();
                var defaultCat = categories.FirstOrDefault() ?? new Category { Name = "General" };

                // 10 Verified, active YouTube videos confirmed to exist and play
                ...
                // Update existing videos with verified ones and distribute across channels
                var existingVideos = await context.videos.ToListAsync();
